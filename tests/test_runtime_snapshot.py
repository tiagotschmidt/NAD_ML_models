...
def temp_snapshot_dir(tmp_path, monkeypatch):
    ...
    snapshot_dir.mkdir()
    monkeypatch.setattr(RuntimeSnapshot, "RUNTIME_SNAPSHOT_DIR", str(snapshot_dir))
...
class TestSnapshotSaving:
    ...
    def test_save_overwrites_existing_file(self, snapshot_instance, temp_snapshot_dir):
        ...
        assert len(os.listdir(temp_snapshot_dir)) == 1
...
class TestSnapshotLoading:
    ...
    def test_load_latest_returns_none_if_dir_not_exists(self, monkeypatch):
        ...
        """
        monkeypatch.setattr(
            RuntimeSnapshot, "RUNTIME_SNAPSHOT_DIR", "./non_existent_dir/"
        )
        assert RuntimeSnapshot.load_latest() is None
    ...
    def test_load_latest_returns_none_for_empty_dir(self, temp_snapshot_dir):
        ...
        """
        assert os.listdir(temp_snapshot_dir) == []
        assert RuntimeSnapshot.load_latest() is None
    ...
    def test_load_latest_finds_the_most_recent_file(self, temp_snapshot_dir):
        ...
        old_snapshot = RuntimeSnapshot("old_model", [], None)  # type: ignore
        old_filepath = os.path.join(
            temp_snapshot_dir, f"old_model_{base_time - 100}.snapshot"
        )
        with open(old_filepath, "wb") as f:
            pickle.dump(old_snapshot, f)

        latest_snapshot = RuntimeSnapshot(
            "latest_model", [1], None, last_profiled_index=99  # type: ignore
        )
        latest_filepath = os.path.join(
            temp_snapshot_dir, f"latest_model_{base_time}.snapshot"
        )
        with open(latest_filepath, "wb") as f:
            pickle.dump(latest_snapshot, f)

        loaded = RuntimeSnapshot.load_latest()
        assert loaded is not None
        assert loaded.model_name == "latest_model"
